...
public class PagerSlidingTabStrip extends HorizontalScrollView {
	...
	private int tabTextColor = R.color.text_tab_press;
	...
	private void onTabTouchStyles(View tab, int position){
		tab.setBackground(getContext().getResources().getDrawable(((IconTabProvider) pager.getAdapter()).getBackgroundPress()));
		ViewGroup tabs = (ViewGroup) tab;

		for(int i = 0; i < tabs.getChildCount(); i++){
			View tabview = tabs.getChildAt(i);
			if(tabview instanceof TextView){
				TextView title = (TextView) tabview;
				title.setTextColor(((IconTabProvider) pager.getAdapter()).getTextColorPressed());
			}

			if(tabview instanceof ImageView){
				ImageView icon = (ImageView) tabview;
				Log.d("Sliding", "Press icon imageview");
				icon.setImageResource(((IconTabProvider) pager.getAdapter()).getPageIconPressResId(position));
				Log.d("Sliding", "Press icon" + ((IconTabProvider) pager.getAdapter()).getPageIconPressResId(position));
			}
		}
	}
	...
	private void updateTabStyles() {
		...
		for (int i = 0; i < tabCount; i++) {
			...
			for (int t = 0; t < ((ViewGroup) v).getChildCount(); t++){

				View vg = ((ViewGroup) v).getChildAt(t);
				if(vg instanceof TextView){
					TextView tab = (TextView) vg;
					tab.setTextSize(TypedValue.COMPLEX_UNIT_PX, tabTextSize);
					tab.setTypeface(tabTypeface, tabTypefaceStyle);
					tab.setTextColor(((IconTabProvider) pager.getAdapter()).getTextColorPress());
				}
				if(vg instanceof ImageView){
					ImageView tab = (ImageView) vg;
					tab.setImageResource(((IconTabProvider) pager.getAdapter()).getPageIconResId(i));
				}

				if(isfrist){
					if(i == 0){
						onTabTouchStyles(v,0);
					}
				}
			}
